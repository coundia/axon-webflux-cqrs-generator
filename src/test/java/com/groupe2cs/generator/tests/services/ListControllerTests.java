...
import com.groupe2cs.generator.service.ListControllerGeneratorService;
...
public class ListControllerTests {
    ...
    @Autowired
    ListControllerGeneratorService service;

    @Autowired
    GeneratorProperties generatorProperties;

    @Test
    void it_should_generate_list_controller_file(@TempDir Path tempDir) throws Exception {
        Path templatesDir = tempDir.resolve("templates");
        Files.createDirectories(templatesDir);

        Files.writeString(
                templatesDir.resolve("listController.mustache"),
        ...
        File file = tempDir.resolve(generatorProperties.getControllerPackage() + "/MockEntityListController.java").toFile();
        assertThat(file).exists();

        String content = Files.readString(file.toPath());
        assertThat(content).contains("class MockEntityListController");
    }
}
