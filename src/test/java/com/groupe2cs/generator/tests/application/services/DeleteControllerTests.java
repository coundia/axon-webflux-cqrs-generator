...
import com.groupe2cs.generator.application.service.DeleteControllerGeneratorService;
...
public class DeleteControllerTests {
    ...
    @Autowired
    DeleteControllerGeneratorService service;

    @Autowired
    GeneratorProperties generatorProperties;

    @Test
    void it_should_generate_delete_controller_file(@TempDir Path tempDir) throws Exception {
        Path templatesDir = tempDir.resolve("templates");
        Files.createDirectories(templatesDir);
        Files.writeString(
                templatesDir.resolve("deleteController.mustache"),
        ...
        File generated = tempDir.resolve(generatorProperties.getControllerPackage() + "/DeleteMockEntityController.java").toFile();
        assertThat(generated).exists();

        String content = Files.readString(generated.toPath());
        assertThat(content).contains("public class DeleteMockEntityController");
    }
}
